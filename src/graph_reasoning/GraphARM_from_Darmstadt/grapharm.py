...
class GraphARM(nn.Module):
    ...
    def __init__(self,
        ...
        self.masker = NodeMasking(dataset, semantics_settings)
    ...
    def preprocess(self, graph):
        ...
        graph = self.masker.process_gw(graph)
        return graph

    def train_step(self, train_don_data, train_deno_data, val_data, M = 4):
        eta1 = 1.
        eta2 = 1.
        ...
        acc_loss = 0.0
        with tqdm(train_don_data) as pbar:
            for graph in pbar:
                # preprocess graph
                ...
                for diffusion_trajectory, node_order, sigma_t_dist in diffusion_trajectories:
                    ...
                    for t in range(len(node_order)):
                        for k in range(len(node_order) - t - 1):# until t
                            ...
                            pbar.set_description(f"DON loss: {acc_loss:.4f}")
        
        # update parameters using accumulated gradients
        self.denoising_optimizer.step()
        
        # log loss
        wandb.log({"DON loss": acc_loss})
        ...
        acc_reward = 0.0
        with tqdm(train_deno_data) as pbar:
            for graph in pbar:
                # preprocess graph
                graph = self.preprocess(graph)
                diffusion_trajectories = self.generate_diffusion_trajectories(graph, M)  
                ...
                for diffusion_trajectory, node_order, sigma_t_dist_unmasked  in diffusion_trajectories:
                    ...
                    for t in range(len(node_order)):
                        for k in range(len(node_order) - t - 1):
                            ...
                            # wandb.log({"target_node_ordering_prob": w_k.item()})
                            # calculate loss
                            reward = self.vlb(G_0, node_type_probs, edge_type_probs, w_k, node_order[k], node_order, t, M) * eta2
                            # wandb.log({"vlb": reward.item()})
                            acc_reward -= reward.item()
                            # backprop (accumulated gradients)
                            reward.backward(retain_graph=True)
                            pbar.set_description(f"DeNo reward: {acc_reward:.4f}")

        
        wandb.log({"DeNo reward": acc_reward})
                ...
                for diffusion_trajectory, node_order, sigma_t_dist in diffusion_trajectories:
                    ...
                    for t in range(len(node_order)):
                        for k in range(len(node_order) - t - 1):# until t
                            ...
                            w_k = w_k.detach()
                            # calculate loss
                            loss = self.vlb(G_0, node_type_probs, edge_type_probs, w_k, node_order[k], node_order, t, M)
                            acc_loss += loss.item()
                            # backprop (accumulated gradients)

        wandb.log({"val loss": acc_reward})
