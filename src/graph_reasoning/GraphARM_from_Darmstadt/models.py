...
class DiffusionOrderingNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        num_node_types=len(semantics_settings["all"]["nodes"]) + 1 # add one for masked node type
        num_edge_types=len(semantics_settings["all"]["edges"]) + 2 # add one for masked edge type and one for empty edge type
        
        num_layers=don_settings["num_layers"]
        self.device = device
        self.hidden_dim = don_settings["hidden_dim"]
        self.out_channels = don_settings["out_channels"]

        # add positional encodings into node features
        self.embedding = nn.Embedding(num_embeddings=num_node_types, embedding_dim=self.hidden_dim).to(self.device)
...
class DenoisingNetwork(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.device = device
        node_feature_dim=deno_settings["node_feature_dim"]
        edge_feature_dim=deno_settings["edge_feature_dim"]
        num_node_types=len(semantics_settings["all"]["nodes"])
        num_edge_types=len(semantics_settings["all"]["edges"]) + 1 # add one for empty edge type
        self.num_layers=deno_settings["num_layers"]
        hidden_dim=deno_settings["hidden_dim"]
        self.K=deno_settings["K"]
        ...
        self.edge_pred_layer = nn.Sequential(Linear(hidden_dim, hidden_dim),
                                       nn.ReLU(),
                                       Linear(hidden_dim, num_edge_types*self.K)).to(self.device)
