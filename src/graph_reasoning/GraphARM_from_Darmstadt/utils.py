...
class NodeMasking:
    def __init__(self, dataset, semantics_settings):
        self.dataset = dataset
        self.semantics_settings = semantics_settings
        self.node_type_to_idx = {node_type: idx for idx, node_type in enumerate(semantics_settings["all"]["nodes"])}
        self.edge_type_to_idx = {edge_type: idx for idx, edge_type in enumerate(semantics_settings["all"]["edges"])}
        self.NODE_MASK = len(semantics_settings["all"]["nodes"])
        self.EMPTY_EDGE = len(semantics_settings["all"]["edges"])
        self.EDGE_MASK = len(semantics_settings["all"]["edges"]) + 1
    ...
    def process_gw(self, nxgraph):
        nodes_attrs = nxgraph.get_attributes_of_all_nodes()
        num_nodes = len(nodes_attrs)
        edges_attrs = nxgraph.get_attributes_of_all_edges()
        num_edges = len(edges_attrs)

        data = Data()
        data.x = torch.tensor([self.node_type_to_idx[node_attrs[1]["type"]] for node_attrs in list(nodes_attrs)]).reshape(-1, 1)
        data.edge_attr = torch.ones([num_nodes, num_nodes]) * self.EMPTY_EDGE

        for edge_attr in edges_attrs:
            data.edge_attr[edge_attr[0], edge_attr[1]] = self.edge_type_to_idx[edge_attr[2]["type"]]

        data.edge_attr = torch.squeeze(data.edge_attr.long().reshape(-1, 1))
        ...
        data.edge_index = torch.tensor([x1, x2])
